...
# Fungsi menghitung Eye Aspect Ratio (EAR) untuk deteksi kedipan mata
def calculate_ear(eye):
    A = distance.euclidean(eye[1], eye[5])  # Jarak vertikal atas-bawah
    B = distance.euclidean(eye[2], eye[4])  # Jarak vertikal tengah-atas dan bawah
    C = distance.euclidean(eye[0], eye[3])  # Jarak horizontal
    ear = (A + B) / (2.0 * C)
    return ear
    ...
    while cap.isOpened():
        ...
        for (x, y, w, h) in faces:
            ...
            if confidence <= 50.0:
                ...
                if len(eyes) == 0:  # Kedipan terdeteksi jika mata tertutup
                    blink_counter += 1
                    cv2.putText(frame, "Eye Blink ✓", (x, y - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                else:
                    blink_counter = 0
                ...
                if smile_detected:
                    cv2.putText(frame, "Toothy Smile ✓", (x, y - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

                # Jika kedipan mata dan senyuman terdeteksi
                if blink_counter >= 1 and smile_detected:
