...
# Variabel untuk deteksi kedipan mata
EYE_CLOSED_FRAMES = 3  # Ambang batas jumlah frame mata tertutup
blink_counter = 0
...
def check_eye_blink(eyes_detected, eye_closed_frames):
    ...
    if eyes_detected == 0:  # Mata tertutup
        ...
        if blink_counter >= eye_closed_frames:  # Kedipan valid jika melebihi ambang batas
            blink_detected = True
        blink_counter = 0  # Reset counter

    if blink_detected:
        blink_detected = False  # Reset flag setelah deteksi
        return True
    return False
    ...
    while cap.isOpened():
        ...
        for (x, y, w, h) in faces:
            ...
            if confidence <= 50.0:
                ...
                if check_eye_blink(len(eyes), EYE_CLOSED_FRAMES):
